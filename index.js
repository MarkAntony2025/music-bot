...
function createQueueEmbed(serverQueue) {
  ...
  return new EmbedBuilder()
    .setTitle('🎧 DJ Music Queue')
    .setColor('Random')
    .setThumbnail(thumbnail)
    .setDescription(`**Now Playing:** 🎶 ${current.title} | ⏱ ${current.duration || 'N/A'} | Requested by <@${current.requester}>\n\n**Up Next:**\n${description}`)
    .setFooter({ text: 'Powered by Premium Music Bot', iconURL: client.user?.avatarURL() });
}
...
async function playSong(guildId) {
  ...
  if (!serverQueue.songs.length) {
    ...
    else { serverQueue.connection.destroy(); queue.delete(guildId); serverQueue.textChannel.send('Queue ended. Leaving VC.'); return; }
  }
